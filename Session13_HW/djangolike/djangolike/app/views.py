...
def detail(request, post_pk):
    ...
    {'post': post, 'my_like':my_like.count(), 'my_scrap':my_scrap.count()})
...
def mypage(request):
    ...
    like_list = likes.filter(user = request.user)
    scraps = Scrap.objects.all()
    scrap_list = scraps.filter(user = request.user)
    return render(request, 'mypage.html', {"posts": posts, "like_list":like_list, "scrap_list": scrap_list,})
